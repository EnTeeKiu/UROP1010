...
class StructuredAgent(TradingAgent):
    ...
    def get_prompt(self):
        ...
        prompt += f"Respond ONLY with a JSON object in this exact format:\n"
        prompt += f"{{\n"
        prompt += f'  "reasoning": "step-by-step logic here",\n'
        prompt += f'  "action": "BUY" | "SELL" | "HOLD",\n'
        prompt += f'  "price": <integer>,\n'
        prompt += f'  "quantity": <integer>\n'
        prompt += f"}}\n"
    ...
    def make_llm_decision(self):
        ...
        try:
            ...
            if action in ["BUY", "SELL"]:
                ...
                if price <= 0 or qty <= 0:
                    raise ValueError("Missing or invalid price/qty for BUY/SELL")
