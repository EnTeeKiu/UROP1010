# Experiment 3: Minimal LLM Baseline
# ============================================
# 10 trading agents in an ABIDES continuous double auction.
# Replace 1 ZIC agent with 1 MinimalAgent.
...
from agent.exp3.MinimalAgent import MinimalAgent

########################################################################################################################
############################################### GENERAL CONFIG #########################################################

parser = argparse.ArgumentParser(description='Experiment 3: Minimal Baseline')
...
num_llm_agents = 1
agents.extend([MinimalAgent(id=j, name="LLM_MINIMAL_{}".format(j), type="MinimalAgent", symbol=symbol, starting_cash=starting_cash, wake_up_freq='60s', q_max=10, log_orders=True, ollama_url='http://localhost:11434/v1', model_name='gemma3:4b', random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2**31 - 1))) for j in range(agent_count, agent_count + num_llm_agents)])
agent_types.extend(["MinimalAgent"] * num_llm_agents)
agent_count += num_llm_agents

kernel = Kernel("Exp3 Minimal Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2**31 - 1)))
