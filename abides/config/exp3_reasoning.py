# Experiment 3: Reasoning LLM Baseline
# ============================================
...
from agent.exp3.ReasoningAgent import ReasoningAgent

parser = argparse.ArgumentParser(description='Experiment 3: Reasoning Baseline')
...
num_llm_agents = 1
agents.extend([ReasoningAgent(id=j, name="LLM_REASONING_{}".format(j), type="ReasoningAgent", symbol=symbol, starting_cash=starting_cash, wake_up_freq='60s', q_max=10, log_orders=True, ollama_url='http://localhost:11434/v1', model_name='gemma3:4b', random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2**31 - 1))) for j in range(agent_count, agent_count + num_llm_agents)])
agent_types.extend(["ReasoningAgent"] * num_llm_agents)
agent_count += num_llm_agents

kernel = Kernel("Exp3 Reasoning Kernel", random_state=np.random.RandomState(seed=np.random.randint(low=0, high=2**31 - 1)))
